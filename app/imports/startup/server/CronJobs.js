...
import { TeamInvitation } from '../../api/team/TeamInvitationCollection';
...
SyncedCron.add({
  ...
  job() {
    const teamInvite = TeamInvitation.find({}).fetch();
    _.forEach(teamInvite, (join) => {
      if (!join.sentDM) {
        ...
        TeamInvitation.update(join._id, { team: join.teamID, developer: join.developerID, sentDM: true });
      }
    });
  },
});
